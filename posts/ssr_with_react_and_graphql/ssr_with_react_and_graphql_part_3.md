...
We will use GraphQL.NET together with WebApi that will be responsible for exposing an endpoint to GraphQL. It should be sufficient to install [GraphQL](https://www.nuget.org/packages/GraphQL/) and [GraphQL.NewtonsoftJson](https://www.nuget.org/packages/GraphQL.NewtonsoftJson/). In addition to this, we also add support for WebApi and make it possible to use dependency injection for these controllers, see feature [WebApi](https://github.com/loremipsumdonec/optimizely-cms-models/tree/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/WebApi) for example.

There are also some services for GraphQL.NET that are good to register for dependency injection and that is `DocumentExecuter` and `DocumentWriter`, see [GraphQLInitialization.cs](https://github.com/loremipsumdonec/optimizely-cms-models/blob/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/GraphQL/Initialization/GraphQLInitialization.cs).
...
A first feature that can be good to start with is support for breadcrumbs. For this we will need a service and two models. If you look at the files [BreadcrumbsService.cs](https://github.com/loremipsumdonec/optimizely-cms-models/tree/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/Breadcrumbs/Models/BreadcrumbsService.cs), [BreadcrumbsModel.cs](https://github.com/loremipsumdonec/optimizely-cms-models/tree/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/Breadcrumbs/Models/BreadcrumbsModel.cs), [Breadcrumb.cs](https://github.com/loremipsumdonec/optimizely-cms-models/tree/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/Breadcrumbs/Models/Breadcrumb.cs), you can see the current implementation. The next step is to create the GraphQL types for the models and below you will see how these could be set up.
...
Before these become available via GraphQL, we need to add two more classes, one class that will be responsible for all queries and then a schema. These classes should also be registered for dependency injection.
...
The final step is then to create an endpoint, see [FirstIterationApiController.cs](https://github.com/loremipsumdonec/optimizely-cms-models/blob/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/FirstIteration/Controllers/FirstIterationApiController.cs) for example. If you then visit the endpoint with [GraphiQL](https://github.com/graphql/graphiql), you should be able to use it.
...
You can find the complete implementation of navigation [here](https://github.com/loremipsumdonec/optimizely-cms-models/tree/master/posts/ssr_with_react_and_graphql/example/lorem_headless/Features/Navigation) and if we use it, it is possible to control how many levels to retrieve.
